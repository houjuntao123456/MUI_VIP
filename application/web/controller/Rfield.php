...
class Rfield extends Common
{
    ...
    public function fieldSel()
    {
        ...
        //模糊查询
        if (input('search') !== '') {
            $where[] = ['name|vgname|vsname|vip_code|vyname|vyphone|vlname', 'like', '%' . input('search') . '%'];
        } else {
            $where = true;
        }
        //查询当前登入人
    ...
    public function fieldStatus()
    {
        ...
        // 获取状态
        $replace =Db::table($this->db.'.vip_field_interaction_foot')->find($id);
        if ($replace['status'] == 1) {
        ...
        try {
            // 执行修改
            Db::table($this->db.'.vip_field_interaction_foot')->update($data);
     ...
     * 修改小专场跟进审核状态
     */
    public function fieldDillStatus()
    {
        $id = input('id') ?? null;
        if ($id == null) {
            webApi(1, '参数错误');
        ...
        // 获取状态
        $replace = Db::table($this->db.'.vip_field_interaction_foot_info')->find($id);
        ...
        // 执行修改
        $res =  Db::table($this->db.'.vip_field_interaction_foot_info')->update($data);
        unset($id,$status,$data,$replace);
        if ($res) {
            webApi(0, 'yes', 0, '修改成功!');
        } else {
    ...
    public function fieldDel()
    {
        ...
        Db::startTrans();
        try {
            Db::table($this->db.'.vip_field_interaction_foot')->delete($id);
            Db::table($this->db.'.vip_field_interaction_foot_info')->where('field_foot_code', $bill)->delete();
    ...
    public function fieldEdit()
    {
        ...
        $res = Db::table($this->db.'.vip_field_interaction_foot')->update($data);
        unset($id,$data);
        if ($res) {
            webApi(0, 'yes', 0, '修改成功!');
        } else {
    ...
    public function fieldDelAll()
    {
        ...
        Db::startTrans();
        try {
            Db::table($this->db.'.vip_field_interaction_foot')->where('id', 'in', $ids)->delete();
            Db::table($this->db.'.vip_field_interaction_foot_info')->where('field_foot_code', 'in', $bills)->delete();
